...
const ExploreByIndustry = () => {
  ...
  useEffect(() => {
    ...
    const onLocale = () => {
      load(); // refetch strapi content on language change
    };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5 justify-items-center">
            {cards.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="w-full max-w-[268px] min-h-[175px] bg-white rounded-2xl px-5 py-6 flex flex-col items-center text-center shadow-lg"
                >
